...
')
  ...
  a.href = url; a.download = 'ssmls_student_import_template.csv'; a.click()
...
export default function ImportStudentsPage() {
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!parsed.length) return toast.error('No data found in file. Check the format.')
      setRows(parsed)
      setStep(2)
    }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.data.results.created > 0)
        toast.success(`${res.data.results.created} student${res.data.results.created!==1?'s':''} imported!`)
      if (res.data.results.skipped > 0)
        toast.error(`${res.data.results.skipped} row${res.data.results.skipped!==1?'s':''} skipped.`)
    } catch (err) {
  ...
  const reset = () => { setStep(1); setRows([]); setFileName(''); setResults(null); if(fileRef.current) fileRef.current.value='' }

  const STRAND_COLOR = { STEM:'bg-blue-100 text-blue-700', HUMSS:'bg-purple-100 text-purple-700',
    ABM:'bg-amber-100 text-amber-700', TVL:'bg-green-100 text-green-700', GAS:'bg-slate-100 text-slate-600' }
        ...
        {[['1','Upload CSV'],['2','Preview'],['3','Results']].map(([n, label], i) => (
          <div key={n} className="flex items-center gap-2">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all
              ${step===Number(n) ? 'bg-primary text-white' : step>Number(n) ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>
              {step>Number(n) ? <CheckCircle size={12}/> : <span className="w-4 text-center">{n}</span>}
              {label}
            </div>
      ...
      {step === 1 && (
        <div className="space-y-5">
          ...
          <div className="card p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>{['Column','Required','Example','Notes'].map(h=>(
                ...
                <tbody className="divide-y divide-slate-50">
                  {[
                    ...
                    ['middle_name', '— No',  'Dela',                    'Optional middle name'],
            ...
            <p className="text-xs text-slate-400 mt-3 bg-amber-50 border border-amber-200 rounded-lg p-2.5">
              <strong className="text-amber-700">Default password:</strong> All imported students get <span className="font-mono font-bold text-amber-700">Student@2026</span> as their initial password. They can change it after first login.
            </p>
          </div>
          ...
          <div
            ...
            className="card p-12 border-2 border-dashed border-slate-200 text-center cursor-pointer hover:border-primary/40 hover:bg-primary/2 transition-all group">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="card p-5 text-center border-emerald-200 bg-emerald-50">
              ...
              <p className="text-xs font-semibold text-emerald-600 mt-1">Successfully Created</p>
            </div>
            <div className="card p-5 text-center border-red-200 bg-red-50">
              <p className="text-3xl font-bold text-red-500">{results.skipped}</p>
              <p className="text-xs font-semibold text-red-500 mt-1">Skipped / Errors</p>
            </div>
            <div className="card p-5 text-center col-span-2 sm:col-span-1">
              ...
              <p className="text-xs font-semibold text-slate-500 mt-1">Total Processed</p>
            </div>
          </div>
          ...
          {results.created > 0 && (
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3">
              ...
              <div className="text-sm text-emerald-700">
                <strong>{results.created} student account{results.created!==1?'s':''}</strong> created successfully.
                Default password: <span className="font-mono font-bold">Student@2026</span>
              </div>
            </div>
          )}
